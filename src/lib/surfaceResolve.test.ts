...
describe('deriveStripTarget', () => {
  it('drives dimmer from the fader and the flash, whatever the encoder bank', () => {
    expect(deriveStripTarget('fader', group, 'pan')).toEqual({
      type: 'groupProperty',
      groupName: 'front-wash',
      propertyName: 'dimmer',
    })
    expect(deriveStripTarget('flash', group, 'pan')).toEqual({
      type: 'flash',
  ...
  it('toggles rather than replaces on the select button', () => {
    expect(deriveStripTarget('select', group, 'dimmer')).toEqual({
      type: 'selectTarget',
      target: group,
      mode: 'toggle',
    })
  ...
  it('follows the encoder bank on the encoder', () => {
    expect(deriveStripTarget('encoder', group, 'dimmer')).toMatchObject({ propertyName: 'dimmer' })
    expect(deriveStripTarget('encoder', group, 'colour')).toMatchObject({ propertyName: 'colour' })
  })

  it('derives fixture targets from a fixture target', () => {
    expect(deriveStripTarget('fader', fixture, 'tilt')).toEqual({
      type: 'fixtureProperty',
      fixtureKey: 'hex-1',
      propertyName: 'dimmer',
    })
    expect(deriveStripTarget('encoder', fixture, 'tilt')).toEqual({
      type: 'fixtureProperty',
      fixtureKey: 'hex-1',
      propertyName: 'tilt',
    })
...
describe('resolveControl', () => {
  it('answers null when nothing is bound', () => {
    expect(resolveControl('fader-1', index([]), null, 'dimmer')).toBeNull()
  })
  ...
  it('prefers the exact bank to the bank-agnostic row on one control', () => {
    ...
    expect(resolveControl('fader-1', i, 'layer-a', 'dimmer')?.binding.id).toBe(2)
    expect(resolveControl('fader-1', i, 'layer-b', 'dimmer')?.binding.id).toBe(1)
  })

  it('resolves each of a strip’s controls to that control’s role', () => {
    const i = index([binding(9, 'strip-1', { type: 'strip', target: group })])
    expect(resolveControl('fader-1', i, null, 'colour')?.target).toEqual({
      type: 'groupProperty',
      groupName: 'front-wash',
      propertyName: 'dimmer',
    })
    expect(resolveControl('enc-1', i, null, 'colour')?.target).toEqual({
      type: 'groupProperty',
      groupName: 'front-wash',
      propertyName: 'colour',
    })
    expect(resolveControl('btn-25', i, null, 'colour')?.target).toEqual({
      type: 'selectTarget',
      target: group,
      mode: 'toggle',
    })
    expect(resolveControl('btn-1', i, null, 'colour')?.target).toMatchObject({ type: 'flash' })
  })
  ...
  it('lets a control’s own binding beat its strip, leaving the siblings on the strip', () => {
    ...
    expect(resolveControl('fader-1', i, null, 'dimmer')?.binding.id).toBe(10)
    expect(resolveControl('enc-1', i, null, 'dimmer')?.binding.id).toBe(9)
  })
  ...
  it('lets a bank-agnostic direct binding beat an exact-bank strip', () => {
    ...
    expect(resolveControl('fader-1', i, 'layer-a', 'dimmer')?.binding.id).toBe(10)
  })
  ...
  it('lets an exact-bank strip beat a bank-agnostic strip', () => {
    ...
    expect(resolveControl('fader-1', i, 'layer-a', 'dimmer')?.target).toMatchObject({
      groupName: 'movers',
    })
    expect(resolveControl('fader-1', i, 'layer-b', 'dimmer')?.target).toMatchObject({
      groupName: 'front-wash',
    })
  })
  ...
  it('carries the strip row’s health to every derived control', () => {
    ...
    expect(resolveControl('enc-1', i, null, 'dimmer')?.binding.health).toEqual({
      type: 'missingGroup',
      groupName: 'front-wash',
    })
  })
  ...
  it('resolves nothing for a control no strip claims', () => {
    ...
    expect(resolveControl('fader-9', i, null, 'dimmer')).not.toBeNull()
    expect(resolveControl('enc-9', i, null, 'dimmer')).toBeNull()
  })
  ...
  it('answers a strip slot holding something other than a strip as it stands', () => {
    ...
    const hit = resolveControl('fader-1', i, null, 'dimmer')
  ...
  it('ignores rows belonging to another device type', () => {
    ...
    expect(resolveControl('fader-1', index([other]), null, 'dimmer')).toBeNull()
  })
})
