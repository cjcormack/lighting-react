...
import { useCopyLookMutation } from '@/store/looks'

interface CopyLookDialogProps {
  open: boolean
...
export function CopyLookDialog({
  open,
  setOpen,
  sourceProjectId,
  lookId,
  ...
  const [copyLook, { isLoading, error, reset }] = useCopyLookMutation()
  ...
  const errorMessage =
    error && 'status' in error && error.status === 409
      ? 'A look with this name already exists in the target project'
      : error
        ? 'Failed to copy look'
        : undefined
  ...
  return (
    <Sheet open={open} onOpenChange={(open) => !open && handleClose()}>
      <SheetContent className="flex flex-col sm:max-w-md">
        <SheetHeader>
          <SheetTitle>Copy Look to Project</SheetTitle>
          <SheetDescription>
            Copy &quot;{lookName}&quot; to another project.
          </SheetDescription>
        </SheetHeader>
        <SheetBody>
          {errorMessage && (
