...
import { useClearLookPreviewMutation, usePreviewLookMutation } from '@/store/looks'
import type { LookPreviewRequest, LookRow, ToggleLookTarget } from '@/api/looksApi'

interface LookLivePreviewProps {
  /** The Look's editor fixture type. Drives the fixture filter. */
  editorFixtureType: string | null
  /** The deferred rows being authored. */
  rows: LookRow[]
  palette: string[]
}
...
/**
 * "Live Preview" panel inside the LookEditor. While enabled, the in-progress draft is pushed to the
 * backend's project-scoped preview slot on every change (debounced); on toggle-off and on unmount
 * the slot is cleared. Group targets are not pre-filtered by fixture type — the backend silently
 * skips members whose typeKey doesn't match.
 *
 * This is what a scratch target set buys a *deferred* Look: its rows name no head, so the only way
 * to see them is to lend them one for as long as the editor is open.
 */
export function LookLivePreview({
  editorFixtureType,
  rows,
  palette,
}: LookLivePreviewProps) {
  const { data: currentProject } = useCurrentProjectQuery()
  const projectId = currentProject?.id

  const { data: fixtureList } = useFixtureListQuery()
  const groups = useGroupListQuery().data ?? []
  const [pushPreview, { isLoading: isPushing }] = usePreviewLookMutation()
  const [clearPreview] = useClearLookPreviewMutation()
  ...
  const compatibleFixtures = useMemo(() => {
    if (!editorFixtureType || !fixtureList) return []
    return fixtureList.filter((f) => f.typeKey === editorFixtureType)
  }, [fixtureList, editorFixtureType])

  const targets = useMemo<ToggleLookTarget[]>(() => {
    const out: ToggleLookTarget[] = []
  ...
  // Skip identical re-pushes — `rows` and `targets` re-identity on every parent render even when
  ...
  const schedulePush = useCallback(() => {
    if (!projectId) return
    // The wire field is `propertyAssignments`, not `rows`: the preview slot is the preset
    // editor's, reused verbatim, which is why the route needed no new backend logic. The target
    // fields are dropped because the slot's targets come from the pills below.
    const payload: LookPreviewRequest = {
      propertyAssignments: rows.map((row) => ({
  ...
  }, [projectId, pushPreview, rows, palette, targets])
    ...
    <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <Button
          ...
          disabled={!editorFixtureType || !projectId}
        >
          {enabled ? 'On' : 'Off'}
        </Button>
      </div>
      ...
      {!editorFixtureType ? (
        <p className="text-[11px] text-muted-foreground">
          Pick an editor fixture type above to enable live preview.
        </p>
      ) : enabled ? (
